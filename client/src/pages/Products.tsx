...
//   return (
//    <div className="min-h-screen pt-20 md:pt-24 relative" data-testid="page-products">
//       <SideBackground />
...
//                     <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-10 ring-inset pt-3">
...
export default function Products() {
  ...
  return (
    <div className="min-h-screen pt-20 md:pt-24 bg-gradient-to-b from-blue-50/20 via-white to-blue-50/10 relative" data-testid="page-products">
      <SideBackground />
      ...
      {/* 2. Main Section: Zedna max-w-full bech na7iw l-espace 📐 */}
      <main className="min-h-screen bg-transparent w-full">
        <div className="flex flex-col md:flex-row w-full mx-auto">

          {/* Sidebar */}
          <div className="md:w-72 flex-shrink-0">
            <div className="sticky top-16 md:top-20 z-10 md:h-[calc(100vh-5rem)]">
              <div className="h-full bg-white/70 backdrop-blur-2xl border-r border-blue-200/30 shadow-2xl overflow-hidden rounded-r-2xl">
                <div className="px-8 py-10 flex flex-col h-full">
                  <FilterBar
                    activeCategory={activeCategory}
                    onCategoryChange={setActiveCategory}
          ...
          {/* 3. Products Grid: Full width responsive 🚀 */}
          <section className="flex-1 pb-10 bg-transparent">
            <div id="products-section" className="w-full px-4 sm:px-6 lg:px-10 py-4">
              {!showLoading && !isError && (
                <div className="flex items-center justify-start mb-8 px-4">
                  <p className="text-lg text-gray-600 font-medium bg-white/40 px-6 py-2 rounded-full backdrop-blur-sm border border-white/50">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-8 pt-3">
                {showLoading ? (
                  Array.from({ length: 15 }).map((_, i) => <ProductSkeleton key={i} />)
                ) : isError ? (
                  <div className="col-span-full text-center py-20">
                    <Package className="w-16 h-16 text-red-500 mx-auto mb-6" />
                    <h3 className="text-3xl font-bold">{t("products.error.title")}</h3>
                  </div>
                ) : products.length > 0 ? (
                  products.map((product) => (
                  ...
                  <div className="col-span-full text-center py-32">
                    ...
                    <h3 className="text-3xl font-bold">{t("products.empty.title")}</h3>
                  </div>
                )}
              </div>
            </div>
