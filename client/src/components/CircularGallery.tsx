...
class App {
  ...
  constructor(container: HTMLElement, config: AppConfig) {
    ...
    this.onResize();
    this.createGeometry();
    this.createMedias(config.items, config.bend, config.textColor || '#fff', config.borderRadius || 0, config.font);
    
    if (this.medias.length > 0) {
      const startPos = 7 * this.medias[0].width; 
      this.scroll.target = startPos;
      this.scroll.current = startPos;
      this.scroll.last = startPos;
    }
  ...
  createMedias(items: any[] | undefined, bend: number = 1, textColor: string, borderRadius: number, font?: string) {
    const galleryItems = items && items.length ? items : [{ image: 'https://picsum.photos/seed/1/800/600', text: '1' }];
    const itemsDoubled = galleryItems.concat(galleryItems);
    this.medias = itemsDoubled.map((data, index) => new Media({
      geometry: this.planeGeometry, gl: this.gl, image: data.image, index, length: itemsDoubled.length,
  ...
  update() {
    // ✅ Auto-scroll
    if (!this.isDown) this.scroll.target += 0.02;
...
addEventListeners() {
  ...
  window.addEventListener('mousemove', (e) => { 
    ...
    this.scroll.target = (this.scroll.position || 0) + (this.start - e.clientX) * (this.scrollSpeed * 0.025); 
  });
  ...
  window.addEventListener('touchstart', (e) => {
    this.isDown = true;
    this.scroll.position = this.scroll.current;
    this.start = e.touches[0].clientX; // Sob3ek win 7atitou
  });
  ...
  destroy() {
    ...
    if (this.gl.canvas.parentNode) this.gl.canvas.parentNode.removeChild(this.gl.canvas as HTMLCanvasElement);
  }
}
...
export default function CircularGallery(props: CircularGalleryProps) {
  ...
  useEffect(() => {
    ...
    const app = new App(containerRef.current, props);
    return () => app.destroy();
  }, [props]);
