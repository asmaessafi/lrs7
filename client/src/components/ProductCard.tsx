...
//       className={cn(
//         "group relative overflow-hidden rounded-3xl bg-white/95 backdrop-blur-md shadow-lg transition-all duration-700 cursor-pointer h-full flex flex-col",
//         isMobile
...
export function ProductCard({ product, onClick }: ProductCardProps) {
  ...
  return (
    ...
    >
      {/* Image Section */}
      <div className={cn("relative aspect-[3/4] overflow-hidden flex-shrink-0", isMobile ? "" : "bg-gradient-to-br from-blue-50/50 to-blue-100/30")}>
        <img
          src={product.imageUrl}
          alt={tProductText(product.id, "name", product.name)}
          className="w-full h-full object-cover transition-transform duration-1000 ease-out group-hover:scale-105"
        ...
        <div className={cn(
            "absolute top-4 left-4 z-20 text-xs sm:text-sm font-bold px-3 py-1 rounded shadow transition-all duration-300",
            isMobile ? (badgeVisible ? "opacity-100 scale-100" : "opacity-0 scale-90") : "opacity-0 group-hover:opacity-100 scale-90 group-hover:scale-100",
            "bg-gradient-to-r", brandColor.bg, brandColor.text, "ring-2 ring-white/50"
          )}
        >
          {product.brand}
        </div>
      ...
      {/* Content Section - Titles now show fully without dots (...) 📝 */}
      <CardContent className="p-4 sm:p-7 space-y-3 sm:space-y-5 bg-gradient-to-b from-white/95 to-blue-50/30 flex-grow flex flex-col">
        <h3 className="font-extrabold text-lg sm:text-xl leading-tight text-blue-950">
          {/* Na7ina line-clamp bech el title yabda kemel */}
          {tProductText(product.id, "name", product.name)}
        </h3>
        <p className="text-sm sm:text-base text-blue-800/80 leading-relaxed font-light">
          {/* Na7ina line-clamp bech el description tabda kemla */}
          {tProductText(product.id, "description", product.description)}
        </p>
      </CardContent>
    </Card>
